      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            aria-modal="true"
            aria-labelledby="exit-title"
            className="bg-background rounded-2xl sm:rounded-3xl max-w-[95vw] sm:max-w-2xl w-full p-6 sm:p-10 md:p-12 relative overflow-y-auto max-h-[90vh]"
            initial={{ y: 20, scale: 0.98, opacity: 0 }}
            animate={{ y: 0, scale: 1, opacity: 1 }}
            exit={{ y: 10, scale: 0.98, opacity: 0 }}
            transition={{ type: "spring", stiffness: 300, damping: 28 }}
          >
            ...
            <button
              onClick={() => setIsOpen(false)}
              className="absolute top-4 right-4 sm:top-6 sm:right-6 p-2 rounded-full hover:bg-accent transition-colors"
              aria-label="Close"
            >
            ...
            <div className="text-center space-y-5 sm:space-y-6">
              {/* Tagline */}
              <div className="mx-auto inline-flex items-center gap-2 rounded-full bg-secondary/10 px-3 py-1.5 sm:px-4 sm:py-2 text-xs sm:text-sm">
                <span className="h-2 w-2 rounded-full bg-secondary" />
                Trusted by hundreds of forward-thinking schools
              </div>
              ...
              {/* Subtitle */}
              <p className="text-base sm:text-lg text-muted-foreground leading-relaxed max-w-xl mx-auto px-1">
                From smarter billing to effortless parent communication — we help schools modernize with tools that
                feel human and truly work.
              </p>
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 pt-2">
                ...
                <button
                  onClick={() => setIsOpen(false)}
                  className="flex-1 border border-border px-6 py-3 sm:px-8 sm:py-4 rounded-full text-base sm:text-lg font-medium hover:bg-accent transition-colors"
                >
                  Continue browsing
                </button>
              </div>
